...
La VM de Asorapa no es accesible por SSH desde Replit ni desde GitHub Actions. Los despliegues se ejecutan manualmente desde una terminal dentro de la propia VM; no intentar despliegues remotos.
...
Para una actualización completa desde `main`, pedir al usuario que ejecute en la VM:

```bash
bash /opt/eqso-asorapa/artifacts/api-server/install/update.sh
```

El script hace `fetch/reset`, instala dependencias, compila y reinicia `eqso.service`.
